      ...
      if(tokenIsValid){
        ...
        _data.read('users',phone,function(err,data){
          if(!err && data){
            ...
            callback(404, {'Error' : 'A user with that phone number does not exist.'})
          }
        })
      } else {
...
// Users - put
// Required data: phone (the user you want to update)
...
handlers._users.put = function(data,callback){
  ...
  if(phone) {
    ...
    if(firstName || lastName || password) {
      ...
      // Verify that th egiven token is valid for the phone number
      handlers._tokens.verifyToken(token,phone,function(tokenIsValid){
        if(tokenIsValid){
          // Lookup user
          _data.read('users',phone,function(err,userData){
            if(!err && userData){
              ...
              _data.update('users',phone,userData,function(err){
                if(!err){
                  callback(200); // update of user data was successful
                } else {
                  callback(500, {'Error': 'Could not update the user'}); // 500 because it might be an error on our end, not an error with the user data being sent
                }
              });
            } else {
                    ...
                    _data.delete('checks', checkId, function(err){
                      ...
                      if(checksDeleted == checksToDelete) {
                        if(!deletionErrors) {
                          ...
                          callback(500, {'Error' : 'Errors encounterd while attempting to delete user checks. Not all checks have been deleted.'})
                        }
                      }
                    })
                  })
                } else {
                  // user doesn't have any checks
                  callback(200);
                }
              } else {
...
handlers._tokens.post = function(data,callback){
  ...
  if(phone && password){
    ...
    _data.read('users',phone,function(err,userData){
      if(!err && userData){
        // Hash the sent password, and compare it to password stored in the user object
        let hashedPassword = helpers.hash(password);
        if(hashedPassword == userData.hashedPassword){
          // If valid, create a new token with a random name. Set expiry date 1 hour in the future.
  ...
  if(id){
    ...
    _data.read('checks',id,function(err,checkData){
      if(!err && checkData){
        ...
        let token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
        console.log(token);
        // verify that the given token and belongs to the user who created the check
    ...
    if(protocol || url || method || successCodes || timeoutSeconds) {
      _data.read('checks',id,function(err,checkData){
        if(!err && checkData) {
          ...
          handlers._tokens.verifyToken(token,checkData.userPhone,function(tokenIsValid){
            if(tokenIsValid){
              // Updat the check where necessary
              if(protocol) {
...
handlers._checks.delete = function(data,callback){
  ...
  let id = typeof(data.queryStringObject.id) == 'string' && data.queryStringObject.id.trim().length == 20 ? data.queryStringObject.id.trim() : false;
  if(id){
    // Lookup the check (to make sure it exists before trying to delete it).
    _data.read('checks', id, function(err, checkData) {
      if(!err && checkData) {
        // Get the token from the headers
        let token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
            ...
            _data.delete('checks', id, function(err){
              if(!err) {
                ...
                _data.read('users',checkData.userPhone,function(err,userData){
                  if(!err && userData) {
                    // find the checks associated with the user
                    let userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
